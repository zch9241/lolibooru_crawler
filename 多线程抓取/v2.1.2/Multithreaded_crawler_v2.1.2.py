...
def Getlastpage():
    html = requests.get(page_links_list[0]).content.decode('utf-8')
    ...
    last_page_number = [links[4]]    #末页所在列表元素
    digitals = re.compile(r'\d+', re.I)
    for i in last_page_number:
        digital = re.search(digitals, i).group()
        if digital:
            digital = int(digital)
    last_page_number_pro = digital

    return last_page_number_pro
